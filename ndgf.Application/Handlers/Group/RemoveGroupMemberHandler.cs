  ...
  ILogger<RemoveGroupMemberHandler> logger)
{
  public async Task<Result<bool>> HandleAsync(RemoveGroupMemberCommand command)
  {
    ...
    if (isArchived)
    {
      logger.LogInformation("[Echec] Tentative de suppression du membre ({MemberToRemove}) du groupe ({GroupId}) par ({RequestingUserId}) échouée - groupe archivé",
    ...
    {
      logger.LogInformation("[Echec] Tentative de suppression du membre ({MemberToRemove}) du groupe ({GroupId}) - ({RequestingUserId}) ne fait pas partie du groupe",
        command.MemberToRemoveId, command.GroupId, command.RequestingUserId);
    ...
    if (!memberToRemoveIsMember)
    {
      logger.LogInformation(
        "[Echec] Tentative de suppression du membre ({MemberToRemove}) du groupe ({GroupId}) par ({RequestingUserId}) échouée - ({MemberToRemoveId}) ne fait pas partie de ce groupe",
    ...
    {
      logger.LogInformation(
        "[Echec] Tentative de suppression d'un membre du groupe ({GroupId}) par ({RequestingUserId}) échouée car il en est le créateur",
    ...
    {
      logger.LogInformation(
        "[Echec] Tentative de supprimer le membre ({MemberToRemove}) du groupe ({GroupId}) par ({RequestingUserId}) échouée - balance du membre non soldée",
        command.MemberToRemoveId, command.GroupId, command.RequestingUserId);
    ...
    {
      logger.LogInformation(
        "[Echec] Tentative de supprimer le membre ({MemberToRemove}) du groupe ({GroupId}) par ({RequestingUserId}) échouée - dernier membre du groupe",
        command.MemberToRemoveId, command.GroupId, command.RequestingUserId);
    ...
    logger.LogInformation(
      "[Succès] Tentative de supprimer le membre ({MemberToRemove}) du groupe ({GroupId}) par ({RequestingUserId}) réussie", command.MemberToRemoveId, command.GroupId,
