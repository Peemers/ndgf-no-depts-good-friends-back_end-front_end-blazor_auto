  ...
  ILogger<ArchiveGroupHandler> logger)
{
  public async Task<Result<bool>> HandleAsync(ArchiveGroupCommand command)
  {
    ...
    if (!isMember)
    {
      logger.LogInformation("[Echec] Tentative d'archivage du groupe ({GroupId}) échouée - ({UserId}) ne fait pas partie du groupe", command.GroupId, command.UserId);
    ...
    if (group is null)
    {
      logger.LogWarning("[Echec] Tentative d'archivage par ({UserId}) échouée le groupe demandé n'existe pas", command.UserId);
    ...
    if (!isBalanceZero)
    {
      logger.LogInformation("[Echec] Tentative d'archivage du groupe par ({UserId}) de ({GroupId}) échouée - groupe non soldé", command.UserId, command.GroupId);
    ...
    logger.LogInformation("[Succès] Tentative d'archivage du groupe ({GroupId}) par ({UserId}) réussie", command.GroupId, command.UserId);
